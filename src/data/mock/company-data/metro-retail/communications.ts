...
export const metroRetailCommunications: Communication[] = Array.from(
  { length: 180 },
  () => {
    const type = faker.helpers.arrayElement(types)
    const status = faker.helpers.arrayElement(statuses)
    const sentDate = faker.date.recent({ days: 90 })
    ...
    const invoiceNumber = faker.datatype.boolean({ probability: 0.75 })
      ? `MR${faker.string.alphanumeric(5).toUpperCase()}`
      : null
    ...
    const amount = parseFloat(faker.finance.amount({ min: 100, max: 8000, dec: 2 }))
