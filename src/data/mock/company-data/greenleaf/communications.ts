...
export const greenleafCommunications: Communication[] = Array.from(
  { length: 65 },
  () => {
    const type = faker.helpers.arrayElement(types)
    const status = faker.helpers.arrayElement(statuses)
    const sentDate = faker.date.recent({ days: 45 })
    const isScheduled = status === 'scheduled'
    const scheduledDate = isScheduled ? faker.date.soon({ days: 20 }) : null

    const customerName = `${faker.person.firstName()} ${faker.person.lastName()}`
    const invoiceNumber = faker.datatype.boolean({ probability: 0.65 })
      ? `GL${faker.string.alphanumeric(5).toUpperCase()}`
      : null
    const invoiceDate = faker.date.past({ years: 1 })
    const dueDate = faker.date.between({
      from: invoiceDate,
      to: new Date(invoiceDate.getTime() + 60 * 24 * 60 * 60 * 1000),
    })
    const amount = parseFloat(faker.finance.amount({ min: 150, max: 3500, dec: 2 }))
