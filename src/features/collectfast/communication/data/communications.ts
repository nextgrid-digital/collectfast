...
const generatePaymentReminderMessage = (customerName: string, invoiceNumber: string, invoiceDate: string, amount: number, dueDate: string, daysOverdue: number) => {
  if (daysOverdue > 0) {
...
export const communications = Array.from({ length: 250 }, () => {
  ...
  const invoiceNumber = faker.datatype.boolean({ probability: 0.7 })
    ? `CF${faker.string.alphanumeric(5).toUpperCase()}`
    : null
